 ...
 * @param  {string}  [config.effect]    切换方式，默认无效果，"none"-无效果，"fade"-淡入淡出，"fadeIn"-仅淡入效果，scrollx"-垂直滚动，"scrolly"-水平滚动，”“-自定义效果
 ...
 *
 * @param  {jQuery}  [config.lists]     内容列表的查询selector，默认为：".j_slideLists"
 * @param  {jQuery}  [config.tags]      标签列表的查询selector，默认为：".j_slideTags"
 * @param  {jQuery}  [config.posCur]    显示当前页码的容器的查询selector，默认为：".j_slideCur"
 * @param  {jQuery}  [config.posPages]  显示总页数的容器的查询selector，默认为：".j_slidePages"
 * @param  {jQuery}  [config.btnPrev]   前一页按钮的查询selector，默认为：".j_slidePrev"
 * @param  {jQuery}  [config.btnNext]   后一页按钮的查询selector，默认为：".j_slideNext"
 *
 * @param  {string}  [config.trigger]   标签的触发方式，默认为“mouseenter”（鼠标滑入切换），也可设置为"click"（点击切换）
 * @param  {string}  [config.tagClass]  标签项选中时附加的类名状态，默认为"current"
 * @param  {string}  [config.btnClass]  按钮被禁用时附加的类名状态，默认为"disabled"
 * @param  {number}  [config.duration]  具有切换效果时的效果持续时间，默认为400（单位：毫秒）
 * @param  {number}  [config.interval]  自动切换时的时间间隔，默认为：5000（单位：毫秒）
 * @param  {number}  [config.cur]       默认显示的内容项的索引值，默认为：0
 * @param  {number}  [config.scrollLen] 每次切换将滚动的单位长度，默认为可视区域的所有列（横向）或所有行（纵向）
 * @param  {number}  [config.hoverDelay] 鼠标悬浮切换时的等待延时，默认不做延时处理
 * @param  {boolean} [config.keepTags]  是否保留标签项的内容，默认为false
 * @param  {boolean} [config.btnDisable] 是否设置当按钮点击到最后一个时进入锁定状态不能点击，默认为false
 * @param  {boolean} [config.beLock]    当目前处于切换运动时，是否将slide置为锁定，避免切换的再次触发，默认为false
 * @param  {boolean} [config.auto]      是否执行自动切换（默认为：false）
 * @param  {boolean} [config.lazyload]  是否启用图片懒加载功能，默认为false-不启用（无缝滚动效果不支持图片懒加载，同时也不建议将图片列表做成无缝滚动效果）
 * @param  {string}  [config.imgAttr]   启用懒加载功能后，用于保存图片真实url的属性名称，默认为："data-slide-img"
 *
